        ...
        "pool_list": [
            {
                ...
                "stop_loss_pct": 0.05, // (可选) 急跌止损阈值：单个轮询周期(~3min)跌幅超过 5% 则撤资并把 token 换回 SOL，默认 0.05
                "vol_filter_pct": 0.03 // (可选) 波动率过滤阈值：单个轮询周期跌幅超过 3% 时本轮不开新仓，默认 0.03
            }
        ]
    }
    ```

    > **无常损失保护**：`stop_loss_pct` 与 `vol_filter_pct` 基于「单个轮询周期内的价格跌幅」触发。
    > 由于一个周期约为 2.5~3.5 分钟，请将阈值设得高于该池的正常成交速度，避免误触发止损。
    > 两个键均为可选；缺省时使用上述默认值。`stop_loss_pct` 触发后会先撤资再通过同一池子把 token 换回 SOL，
    > `vol_filter_pct` 则在市场仍在急跌时阻止重新进场（接飞刀）。
